...
/**************************************   OtherInit  **r************************************/
    ...
    //将标志位全都置0
    memset(&DataLebal,0,sizeof(DataLebal));
}
...
static void VisionJudge()
{
    ...
    {
        DataLebal.vision_flag=1;
        if(abs(minipc_recv_data->Vision.pitch)<5&&abs(minipc_recv_data->Vision.yaw)<5)
        {
            DataLebal.fire_flag=1;
            AlarmSetStatus(aim_success_buzzer, ALARM_ON);
            //离装甲板距离较近时，开火
            aim_success_buzzer->loudness=0.5;
            DataLebal.repare_flag=0;
        }
        else
        {
            DataLebal.fire_flag=0;
            DataLebal.repare_flag=1;
            AlarmSetStatus(aim_success_buzzer, ALARM_OFF);
        }
    }
     //检测不到装甲板，关蜂鸣器，关火
    else if(minipc_recv_data->Vision.deep==0 && DataLebal.vision_flag==1)       
    {
        DataLebal.fire_flag=0;
        DataLebal.repare_flag=0;
        AlarmSetStatus(aim_success_buzzer, ALARM_OFF);
...
{
    chassis_cmd_send.vx = 50.0f * (float)rc_data[TEMP].rc.rocker_left_y; // _水平方向
    chassis_cmd_send.vy =-50.0f * (float)rc_data[TEMP].rc.rocker_left_x; // 竖直方向
...
{
    if(mid_round_patrol.into_flag==0)
    {
        mid_round_patrol.into_flag=1;
        mid_round_patrol.yaw = mid_round_patrol.yaw_init;
    }
    mid_round_patrol.yaw_total_angle= 70.0f+round_patrol.total_round*360+mid_round_patrol.yaw;
...
static void RoundPatrol()
{
    ...
    gimbal_cmd_send.yaw+=0.23;
    // mid_round_patrol.into_flag=0;
}
...
static void TemporaryPatrol()
{
    ...
    if(tem_round_patrol.yaw > tem_round_patrol.yaw_init+1)
    {
        tem_round_patrol.yaw =  tem_round_patrol.yaw_init+1;
        tem_round_patrol.num++;
        tem_round_patrol.direction = -1; // 改变方向
    }
    else if(tem_round_patrol.yaw <- 1.0f+tem_round_patrol.yaw_init)
    {
        tem_round_patrol.yaw=-1+tem_round_patrol.yaw_init;
        tem_round_patrol.num++;
        tem_round_patrol.direction = 1; // 改变方向
    }
...
static void GimbalAC()
{
    GetGimbalInitImu();
    //没发信息时巡逻
    if(DataLebal.vision_flag==0)
    {
        DataLebal.t_pitch = (float32_t)DWT_GetTimeline_s();
        //上方不扫，减少扫描范围
        gimbal_cmd_send.pitch =20*abs(sin(4.0*DataLebal.t_pitch));

        // if (minipc_recv_data->Nav.patrol_mode==0)
        // {

        if (chassis_fetch_data.remain_HP<20&&chassis_fetch_data.game_progress==4)
        {
        ...
        // }
        // else if (minipc_recv_data->Nav.patrol_mode==1)
        // {
        //     MidRoundPatrol();
        // }
    }
...
static void ShootAC()
{
    if(shoot_fetch_data.over_heat_flag!=1&&DataLebal.fire_flag==1)
    {
        shoot_cmd_send.load_mode=LOAD_BURSTFIRE;
    }
    else if(DataLebal.repare_flag==1&&DataLebal.fire_flag==0)
    {
        shoot_cmd_send.load_mode =LOAD_REVERSE;
    }
    else
